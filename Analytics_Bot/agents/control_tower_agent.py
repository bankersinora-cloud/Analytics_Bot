...
from task_routers.control_tower_task_decide import task_decider
from config.allowed_tables import CONTROL_TOWER_TABLES
...
def run_control_tower_agent(query, memory):
    agent = build_agent(engine, CONTROL_TOWER_TABLES, memory)
    ...
    try:
        ...
        logger = create_logs("control_tower")
        ...
        if not used_sql or task_type == "ANALYSIS_ONLY":
            ...
            analysis_prompt = f"""
            You are TAABI's Logistics Alerting & Notification System Expert.

            YOU MUST ANSWER USING ONLY THIS DOMAIN:
            - How alert thresholds work (halt mins, overspeed limit, deviation km)
            - How channel mapping works (email/sms/push/whatsapp)
            - Alert triggering → alert_logs → redis state
            - control_tower_indents stage behaviour
            - control_tower_vehicles SLA rules
            - Notification engine (sms/email queues, templates, retries, failures)

            STRICT RULES:
            - No SQL in this section
            - No programming help
            - No generic knowledge
            - Only TAABI Alerting + CT Indent/Vehicle + Notification system logic
            ...
            Give the correct operational explanation:
            """
